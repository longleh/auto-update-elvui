...
export const getElvUIVersion = async () => {
  let elvuiMetadataFile = getWowFolder().concat(
    "/_retail_/Interface/Addons/ElvUI/ElvUI_Mainline.toc"
  );
  if (!fs.existsSync(elvuiMetadataFile))
    elvuiMetadataFile = getWowFolder().concat(
      "/_retail_/Interface/Addons/ElvUI/ElvUI-Mainline.toc"
    );
