...
const getElvUIVersion = async(wowFolder) => {
    const elvuiMetadataFile = wowFolder.concat('/_retail_/Interface/Addons/ElvUI/ElvUI_Mainline.toc')
    return new Promise((resolve, reject) => {
        fs.readFile(elvuiMetadataFile, 'utf8', function (err, data) {
            ...
            if (versionLineSplitted.length < 1) return reject('Cannot parse ElvUI version')
            return resolve(Number.parseFloat(versionLineSplitted[1]))
          });
    })
}

const getDownloadLine = async() => {
    const { data } = await axios.get(`${config.elvui_website}/download.php?ui=elvui`)
    const dataLines = data.split('\n')
    return dataLines.find((line) => line.includes('Download ElvUI'))
}

const getOnlineElvuiVersion = async () => {
    ...
    if (splittedLine.length < 1) throw Exception('Cannot parse ElvUI online version')
    const versionLine = splittedLine[1].trim()
    let version = ''
    let i = 0;
    while (i < versionLine.length && (versionLine[i].match(/^[0-9a-z]+$/) || versionLine[i] === '.')) {
...
const downloadElvUI = async (onlineVersion) => {
    console.info('Downloading Elvui')
    const htmlLines = await getDownloadLine()
    const linkLines = htmlLines.split('"')
    if (linkLines.length === 0) throw Exception('Cannot parse ElvUI download link')
    const downloadLink = linkLines.find((line) => line.includes(onlineVersion + '.zip'))
...
const start = async() => {
    ...
    const wowFolders = fs.readdirSync(wowFolder.concat('/_retail_/Interface/Addons'))
    const isElvuiInstalled = !!(wowFolders.find((folder) => folder === 'ElvUI') && wowFolders.find((folder) => folder === 'ElvUI_OptionsUI'))
    if (isElvuiInstalled) {
        ...
        try {
            ...
            if (version < onlineVersion) {
                ...
                fs.unlink(fileName, (err) => {
                    if (err) console.error('Could not remove zip file of ElvUI you may want to delete it manually')
                })
            } else {
